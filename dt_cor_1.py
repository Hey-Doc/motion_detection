...
#functions
def dot(u,v): 
    return u['X_value']*v['X_value']+u['Y_value']*v['Y_value']+u['Z_value']*v['Z_value']


#data loading
fall_acc = pd.read_csv("dataset/cor-fall-1_acc.csv")
nfall_acc = pd.read_csv("dataset/cor-notfall-1_acc.csv")
fall_mag = pd.read_csv("dataset/cor-fall-1_mag.csv")
nfall_mag = pd.read_csv("dataset/cor-notfall-1_mag.csv")
...
vf=fall_mag.rolling(window=5,min_periods=1).mean()
vnf=nfall_mag.rolling(window=5,min_periods=1).mean()
"""
...
"""
vnf=nf_acc.rolling(window=500,min_periods=1).mean()
"""
f_acc=f_acc.dropna(axis=0)
nf_acc=nf_acc.dropna(axis=0)
vf=vf.dropna(axis=0)
...
#file export
dataset_fall.to_csv('./formatted_dataset/data_fall_4.csv', sep=',',na_rep=0)
dataset_nfall.to_csv('./formatted_dataset/data_nfall_4.csv', sep=',',na_rep=0)
dataset_net.to_csv('./formatted_dataset/data_net_4.csv',sep=',',na_rep=0)